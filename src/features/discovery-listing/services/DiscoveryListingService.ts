...
export class DiscoveryListingService {
  ...
  private async saveSearchToHistory(searchQuery: ProductSearchQuery, resultCount: number): Promise<void> {
    try {
      ...
      await AsyncStorage.setItem(
        DiscoveryListingService.STORAGE_KEYS.SEARCH_HISTORY,
  ...
  async getSearchHistory(): Promise<SearchHistory[]> {
    try {
      const historyData = await AsyncStorage.getItem(DiscoveryListingService.STORAGE_KEYS.SEARCH_HISTORY);
  ...
  async clearSearchHistory(): Promise<void> {
    try {
      await AsyncStorage.removeItem(DiscoveryListingService.STORAGE_KEYS.SEARCH_HISTORY);
    } catch (error) {
